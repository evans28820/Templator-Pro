...
function generateScanJsx(aiPath: string, outputPath: string, pngPath: string): string {
  return [
    ...
    '// Export preview — fallback chain',
    'var pngOk = false;',
    'var pngFile = new File("' + escapeJsx(pngPath) + '");',
    '',
    '// Attempt 1: PNG24 with full options',
    'try {',
    '  var popts = new ExportOptionsPNG24();',
    '  popts.antiAliasing = true;',
    '  popts.artBoardClipping = true;',
    '  popts.transparency = true;',
    '  doc.exportFile(pngFile, ExportType.PNG24, popts);',
    '  pngOk = pngFile.exists;',
    '} catch(e) {}',
    '',
    '// Attempt 2: PNG24 without options',
    'if (!pngOk) {',
    '  try { doc.exportFile(pngFile, ExportType.PNG24); pngOk = pngFile.exists; } catch(e) {}',
    '}',
    '',
    '// Attempt 3: Save-for-Web PNG',
    'if (!pngOk) {',
    '  try {',
    ...
    '    sfw.PNG8 = false;',
    '    sfw.optimized = true;',
    '    doc.exportFile(pngFile, ExportType.SAVEFORWEB, sfw);',
    '    pngOk = pngFile.exists;',
    '  } catch(e) {}',
    '}',
    '',
